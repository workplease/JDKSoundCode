...
{
    /**
     * 用于维护treeMap集合中的顺序，如果为null，则按照key的自然顺序排序
     *
     ...
     */
    //无参构造函数，将比较器comparator置为null，表示按照key的自然顺序进行排序
    public TreeMap() {
        comparator = null;
    }
    ...
    public V put(K key, V value) {
        ...
        //并且设置集合大小为size，以及modCount+1，这是用于快速校验失败
        if (t == null) {
        ...
        Comparator<? super K> cpr = comparator;
        //如果比较器不为空，即初始化TreeMap构造函数时，则传递Comparator类
        //那么插入新元素时，按照comparator实现的类进行排序
        if (cpr != null) {
        ...
        //如果比较器为空，即初始化TreeMap构造函数时，没有传递comparator类
            ...
            //如果key==null，直接抛出异常
            //上面的comparator传入了比较器，可以允许key=null的情况出现
            if (key == null)
                throw new NullPointerException();
            @SuppressWarnings("unchecked")
                Comparable<? super K> k = (Comparable<? super K>) key;
    ...
    private void deleteEntry(Entry<K,V> p) {
        ...
        //待删除节点只有一个子节点，直接删除该节点，并用该节点的唯一子节点替代该节点
        if (replacement != null) {
            ...
            //TreeMap中只有待删除节点p，也就是只有一个节点，直接返回null即可
